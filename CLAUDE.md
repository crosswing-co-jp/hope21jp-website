...
ユーザーの初回メッセージが**プロジェクト初見と思われる内容**（例: 「はじめまして」「このリポジトリ何？」「何をすればいい？」「オンボーディング」「help」等）の場合、**最初に `/welcome` スラッシュコマンドの内容を表示**してから本題に回答すること。

また、ユーザーが **`/welcome` と入力** した場合も同じコマンド内容を表示する。

内容は `.claude/commands/welcome.md` にある。
